...
<AccordionGroup>
  ...
  <Accordion title="Tools not appearing in Cursor Chat">
    If you edited `mcp.json` manually, make sure Cursor picked up the new config. A restart is sometimes required after adding a new server. Check that `api.nitrosend.com` is reachable: `curl -I https://api.nitrosend.com/mcp`
  </Accordion>
</AccordionGroup>
