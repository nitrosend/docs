...
The connection test does more than read your send quota: it also provisions the integration's bounce and complaint feedback pipeline (an SNS topic and an SES configuration set), so the IAM user needs SES and SNS management permissions from the very first **Test connection**. Adding a sending domain additionally creates and verifies SES identities. A least-privilege policy is possible, but it must cover quota reads, sending, identity creation and verification, configuration-set management, and SNS topic creation and subscription — anything narrower fails either the connection test or domain setup.
