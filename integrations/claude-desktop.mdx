...
<Tip>
  The Custom Connector method is the recommended way to connect. It's simpler than editing config files and keeps your API key secure.
</Tip>

## What Claude can do with Nitrosend connected

Once connected via MCP, Claude has access to **16 specialized tools** and **live resources** about your account. Here's what that means in practice:
...
| `nitro://account/state` | Account status, tier, and sending limits |
...
If you prefer to configure MCP manually (or if the Custom Connector method isn't available in your version), edit your Claude Desktop config file:
