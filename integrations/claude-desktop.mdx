...
<Info>
  Claude Desktop users should connect Nitrosend as a Custom Connector. Do not install `@nitrosend/cli` or run `nitrosend login` for the standard Claude Desktop setup — the CLI is for terminal and CI workflows, not Desktop connector onboarding.
</Info>
...
<Steps>
  <Step title="Open Claude Desktop Settings">
    In the Claude Desktop app, go to **Settings** → **Connections** → **Customize**.
  </Step>
...
- **Set up your Brand Kit first.** Go to [Brand Kit](https://app.nitrosend.com/my/brand) and configure your brand colors, fonts, and logo. Claude uses this automatically when composing emails.
...
If the Custom Connector method isn't available in your version of Claude Desktop, update Claude Desktop first. Only use this local stdio fallback when you cannot use Custom Connectors and you understand that it stores an API key in your local Claude config:
...
<Info>
  Requires Node.js 18 or later for the manual config method. The Custom Connector method has no local dependencies and remains the recommended path.
</Info>
