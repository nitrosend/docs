...
In multi-brand accounts, MCP has a current brand. Use `nitro_get_status` to inspect `current_brand` and `available_brands`, then use `nitro_select_brand` to switch the current brand for OAuth sessions. `nitro_set_brand` does not switch context; it updates the identity of the currently selected brand.
