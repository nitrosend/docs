...
<Warning>
  The MCP call stores the key without testing it. `{"operation": "status"}` reports stored configuration only — to actually test the credentials, run **Test connection** from the Resend integration page in the dashboard.
</Warning>
