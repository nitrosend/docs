...
Get your API key from **Settings > API Keys** in the Nitrosend dashboard ([app.nitrosend.com/my/settings/api-keys](https://app.nitrosend.com/my/settings/api-keys)).
...
projection: the account's fixed plan/standing allowance, plus pacing and a
bounded public reason/remediation. This endpoint never
...
and list its retained per-candidate items. Validation does not prove consent;
qualifying current results contribute to the one standing rank that selects the
plan allowance.
