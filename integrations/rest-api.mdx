...
Get your API key from **Settings > API Keys** in the [Nitrosend dashboard](https://app.nitrosend.com/my/settings/api-keys).
...
## Plan checkout and prepaid funding

Plan purchase and prepaid balance are separate account-scoped lifecycles. Send
a stable `Idempotency-Key` when creating either purchase. For a plan, call
`POST /v1/my/subscription/checkout`, complete provider approval when requested,
then poll `GET /v1/my/subscription/checkout_status?purchase_id=...` until the
requested plan is active.

For prepaid balance, inspect `GET /v1/my/billing/funding` first. Create a
funding purchase with `POST /v1/my/billing/funding/purchases`, passing
`amount_cents`, `currency`, and a stable `Idempotency-Key`. Poll
`GET /v1/my/billing/funding/purchases/{id}` until the purchase is credited.

Checkout creation, browser redirect, and webhook delivery alone are not
completion proof. [Payments for agents](/guides/agent-payments) owns the full
MCP, CLI, and REST journey; [Purchase lifecycle](/concepts/payment-lifecycle)
defines state and authority. Use the generated [API reference](/api/) for exact
fields.
...
|--------|--------|----------|
| **Billing** | | |
| Create or reuse plan checkout | `POST` | `/subscription/checkout` |
| Reconcile plan checkout | `GET` | `/subscription/checkout_status` |
| Get prepaid funding status | `GET` | `/billing/funding` |
| Create or reuse funding purchase | `POST` | `/billing/funding/purchases` |
| Reconcile funding purchase | `GET` | `/billing/funding/purchases/{id}` |
| **Messages** | | |
