...
[Codex CLI](https://github.com/openai/codex) is OpenAI's terminal-based coding agent. Connect it to Nitrosend via remote MCP for access to every public Nitrosend tool, or use the REST API with a helper script for non-OAuth automation.

<Info>
  This guide is for Codex CLI. ChatGPT has a separate official Nitrosend app; see the [ChatGPT guide](/integrations/chatgpt).
</Info>

## Option 1: Codex CLI (MCP server, recommended)
