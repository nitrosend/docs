...
[Codex CLI](https://github.com/openai/codex) is OpenAI's terminal-based coding agent. Connect Nitrosend via MCP for full access to all 23 tools, or use the REST API with a helper script.

<Tip>
  Using Codex inside ChatGPT (not the terminal CLI)? Connect the official Nitrosend app from the [ChatGPT app marketplace](/integrations/chatgpt) instead. The steps below are for the Codex CLI.
</Tip>
