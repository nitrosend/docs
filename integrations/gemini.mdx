...
Gemini CLI has native MCP support, giving you the full Nitrosend toolkit — every public tool, live Brand Kit context, and account state — directly in your terminal.
...
Gemini now has access to the full public set of Nitrosend MCP tools — composing emails, managing contacts, building flows, checking analytics, and more.

<Tip>
  MCP gives Gemini your full Brand Kit context — colors, fonts, tone of voice — so emails it creates are on-brand without extra prompting.
</Tip>

## Example workflows

### Set up Brand Kit + compose

```bash
gemini "Scrape my Brand Kit from https://mysite.com and draft a product announcement email"
```
...
For programmatic integration or Google AI Studio, you can define Nitrosend operations as function declarations and call the REST API directly. This requires an API key from **Settings > API Keys** in the Nitrosend dashboard ([app.nitrosend.com/my/settings/api-keys](https://app.nitrosend.com/my/settings/api-keys)).
...
NITROSEND_API_KEY = "nskey_live_your_key_here"  # From Settings > API Keys: https://app.nitrosend.com/my/settings/api-keys
