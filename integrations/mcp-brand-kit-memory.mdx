...
**Brand Kit** defines how your emails look and who they come from. When your AI creates an email via MCP, these settings are applied automatically — the right colors, fonts, logo, and tone, every time.

In accounts with multiple client accounts, MCP has a current client account. Use `nitro_get_status` to inspect `current_client_account` and `available_client_accounts`, then use `nitro_select_client_account` to switch the current client account for OAuth sessions. `nitro_set_brand_kit` does not switch context; it updates the Brand Kit for the currently selected client account.
...
<Tabs>
  <Tab title="Dashboard">
    Go to [**Client Account > Brand Kit**](https://app.nitrosend.com/my/client-account) in the Nitrosend dashboard. Fill in your Brand Kit details and click save.
  </Tab>
...
<Tabs>
  <Tab title="Dashboard">
    Go to [**Client Account > AI Memory**](https://app.nitrosend.com/my/client-account/memory) in the Nitrosend dashboard. Edit the markdown document and save.
  </Tab>
  <Tab title="Via MCP (AI prompt)">
    Ask your connected AI assistant:
    ```
