...
For accounts with multiple client accounts, ask Claude to check `nitro_get_status` first. OAuth sessions can use `nitro_select_client_account` to persist the selected client account for this user, OAuth application, and account. API keys are pinned to their key's client account, and `nitro_set_brand_kit` edits Brand Kit identity rather than switching context.
