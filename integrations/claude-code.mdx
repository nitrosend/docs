...
For multi-brand accounts, ask Claude to check `nitro_get_status` first. OAuth sessions can use `nitro_select_brand` to persist the selected brand for this user, OAuth application, and account. API keys are pinned to their key's brand, and `nitro_set_brand` edits brand identity rather than switching context.
