...
**MCP (Model Context Protocol)** is an open standard that lets AI tools connect to external services. When you add Nitrosend as an MCP server to Claude Code, Claude gets native access to 23 email marketing tools — no API wrappers, no middleware, no boilerplate.
...
| `nitro_ingest_image` | Import image media by small inline data, public URL, or direct-upload signed_id |
