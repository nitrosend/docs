...
In **Brand > Domains**, prepare the protected Nitrosend address reserved for
your brand, or add the DNS records for a customer-owned sending subdomain. The
brand-subdomain address uses no provider resources until you choose **Prepare sender**.
When it is ready, select its From address explicitly.
...
Live email requires one ready, selected sender. Prepare the reserved Nitrosend
brand-subdomain sender with `POST /v1/my/brands/{sid}/prepare_sending`, or
verify a domain you own. If an eligible first request starts brand-subdomain
preparation, the API returns
`sender_identity_provisioning` and stores no message; retry the same
idempotency key after readiness and selection.
