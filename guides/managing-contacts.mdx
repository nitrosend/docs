...
`contact_ids` is required — omit it and the call fails rather than silently tagging every contact in the client account. Pass `dry_run: true` alongside to preview the affected `contact_count` without persisting.
