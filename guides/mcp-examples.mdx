...
```json
{
  "result": {
    ...
    "delivery": {
      ...
      "pacing_state": {
        "status": "ready",
        "policy_version": "delivery-pacing-v2",
        "scopes": []
      },
    ...
    "issues": [
      {
        "area": "domain",
        "severity": "high",
        "message": "No ready sending identity. Prepare your reserved Nitrosend brand subdomain, or verify a custom domain.",
        "brand_subdomain_status": "not_prepared",
...
prepare the permanently reserved sender or connect a customer-owned domain. For
the Nitrosend path it calls `nitro_manage_domains` with
`operation: "prepare_brand_subdomain"`, polls status, and then calls
`operation: "select_brand_subdomain"` to select the ready sender explicitly.
Preparation retains no email.
