...
### GitHub Actions: draft a release campaign

Use `nitrosend mcp tools call` to create a reviewed campaign draft after a deploy. `--machine` gives stable JSON, exit codes, and an auto-issued idempotency key.

```yaml
name: Draft release campaign
on:
  release:
    types: [published]
  ...
  notify:
    ...
    steps:
      ...
      - name: Draft release email
        ...
        run: |
          nitrosend mcp tools call nitro_compose_campaign --machine --args "$(cat <<EOF
          {
            "name": "Release ${{ github.event.release.tag_name }}",
            "channel": "email",
            "subject": "Release ${{ github.event.release.tag_name }}",
            "body": "Release notes are live: ${{ github.event.release.html_url }}",
...
The `idempotency_key` inside `--args` is what the platform dedupes against — set it from a stable, retry-survivable value like the release ID so a re-run of the job won't create duplicate drafts. Branch on exit codes for retry strategy: `75` retriable, `77` means rotate the API key, `78` means upgrade the CLI step.
...
### Purchase a plan or add prepaid funds

The Nitrosend CLI invokes `nitro_manage_billing` over MCP directly. No
payment-specific CLI or external runtime sits between the command and
Nitrosend.

~~~bash
# Inspect account route, plan, and prepaid balance
nitrosend mcp tools call nitro_manage_billing --machine \
  --args '{"operation":"status"}'

# List plans, then prepare owner-approved checkout
nitrosend mcp tools call nitro_manage_billing --machine \
  --args '{"operation":"plans"}'
nitrosend mcp tools call nitro_manage_billing --machine \
  --args '{"operation":"checkout","params":{"plan_id":2},"idempotency_key":"plan-pro-acme-2026-08-31"}'

# Or prepare a separate prepaid funding purchase
nitrosend mcp tools call nitro_manage_billing --machine \
  --args '{"operation":"add_funds","params":{"amount_cents":5000,"currency":"USD"},"idempotency_key":"fund-validation-acme-2026-08-31"}'
~~~

The command returns an approval URL and purchase ID when provider approval is
needed. The account owner completes Checkout in a browser. Poll
`checkout_status` until the plan is `active`, or
`funding_purchase_status` until funds are `credited`.

See [Payments for agents](/guides/agent-payments) for complete commands, Link
and Stripe CLI terminology, and recovery rules.
