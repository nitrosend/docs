...
For per-call idempotency that survives runner restarts (the common CI case), pass an `idempotency_key` inside `--args` for the underlying MCP write tool, such as `nitro_compose_campaign` or `nitro_compose_flow`. The platform dedupes on that key, not on the CLI's per-process key.

```bash
nitrosend mcp tools call nitro_compose_campaign --machine --args '{
  "name": "Release announcement",
  "channel": "email",
  "subject": "Release notes are live",
  "body": "The latest release is now available.",
  "idempotency_key": "release-announcement-42"
}'
```
